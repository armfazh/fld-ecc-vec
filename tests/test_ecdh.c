...
static void test0_rfc7748_x448(const X_ECDH *x448) {
  printf("==== RFC7748 Test 0 ==\n");
  ...
  x448->shared(shared, input_coord0, key0);
  printf(LABEL, memcmp(output_coord0, shared, x448->key_size) == 0 ? OK : ERROR);
  ...
  x448->shared(shared, input_coord1, key1);
  printf(LABEL, memcmp(output_coord1, shared, x448->key_size) == 0 ? OK : ERROR);
  ...
  x448->keygen(alice_public, alice_sk);
  printf(LABEL, memcmp(alice_pk, alice_public, x448->key_size) == 0 ? OK : ERROR);
  ...
  x448->keygen(bob_public, bob_sk);
  printf(LABEL, memcmp(bob_pk, bob_public, x448->key_size) == 0 ? OK : ERROR);
  ...
  x448->shared(bob_shared_secret, alice_public, bob_sk);
  x448->shared(alice_shared_secret, bob_public, alice_sk);
  printf(LABEL, memcmp(alice_shared_secret, bob_shared_secret, x448->key_size) == 0 ? OK : ERROR);
  printf(LABEL, memcmp(alice_shared_secret, shared_secret, x448->key_size) == 0 ? OK : ERROR);
}

static void times(uint64_t n, argECDHX_Key k, uint8_t x_coordinate, const X_ECDH *ecdh) {
  argECDHX_Key r = ecdh->allocKey();
  argECDHX_Key u = ecdh->allocKey();
  uint64_t i;
...
static void test1_rfc7748_x25519(const X_ECDH *x25519) {
...
static void test1_rfc7748_x448(const X_ECDH *x448) {
  X448_KEY k_1_times = {\
  ...
  printf("==== RFC7748 Test 1 ==\n");
  times(1, k, 5, x448);
  printf(LABEL, memcmp(k, k_1_times, x448->key_size) == 0 ? OK : ERROR);
  times(1000, k, 5, x448);
  printf(LABEL, memcmp(k, k_1000_times, x448->key_size) == 0 ? OK : ERROR);
  /*times(1000000,k,5,x448);
  printf(LABEL,memcmp(k, k_1000000_times, x448->key_size)==0?OK:ERROR);*/
}

static void test_dh(const X_ECDH *ecdh) {
  ...
  cnt = 0;
  test = 0;
  argECDHX_Key alice_private_key = ecdh->allocKey();
  argECDHX_Key alice_session_key = ecdh->allocKey();
  argECDHX_Key alice_shared_key = ecdh->allocKey();
  argECDHX_Key bob_private_key = ecdh->allocKey();
  argECDHX_Key bob_session_key = ecdh->allocKey();
  argECDHX_Key bob_shared_key = ecdh->allocKey();

  for (i = 0; i < TIMES; i++) {
    ecdh->randKey(alice_private_key);
    ecdh->keygen(alice_session_key, alice_private_key);

    ecdh->randKey(bob_private_key);
  ...
  ecdh->freeKey(alice_private_key);
  ecdh->freeKey(alice_session_key);
  ecdh->freeKey(alice_shared_key);
  ecdh->freeKey(bob_private_key);
  ecdh->freeKey(bob_session_key);
...
void test_x25519() {
  ...
  printf("===== X25519 x64 =====\n");
  test_dh(&X25519_x64);
  test0_rfc7748_x25519(&X25519_x64);
  test1_rfc7748_x25519(&X25519_x64);
}
