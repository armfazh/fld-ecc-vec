...
static std::ostream &operator<<(std::ostream &os, const Ed25519_PublicKey &key) {
  unsigned int i = 0;
...
static std::ostream &operator<<(std::ostream &os, const Ed25519_Signature &sig) {
  unsigned int i = 0;
...
//    std::cout << "Test: " << test->name_test << std::endl;
    ...
    switch (test->signature_scheme) {
      case EDDSA_Ed25519:
        ...
        EXPECT_EQ(memcmp(run_signature, test->signature, Ed25519.Pure.signature_size), 0)
                  << "got: " << run_signature << "want: " << test->signature;

        valid = (EDDSA_FLAGS) Ed25519.Pure.verify(test->message, test->message_length, run_publickey, run_signature);
