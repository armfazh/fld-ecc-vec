...
static void times(int n, uint8_t *k, const X_ECDH &x25519) {
  X25519_KEY r, u;
  int i;
  for (i = 0; i < ECDH25519_KEY_SIZE_BYTES; i++) {
    u[i] = 0;
  ...
  for (i = 0; i < n; i++) {
    x25519.shared(r, u, k);
    memcpy(u, k, ECDH25519_KEY_SIZE_BYTES);
    memcpy(k, r, ECDH25519_KEY_SIZE_BYTES);
  }
}

class Vector : public ::testing::TestWithParam<X_ECDH> {
  //  virtual void SetUp() { printf("Setup\n"); }
  //  virtual void TearDown() {printf("TearDown\n");  }
};

TEST_P(Vector, NACL_CRYPTO) {
  const X_ECDH &x25519 = GetParam();
...
TEST_P(Vector, DIFFIE_HELLMAN) {
  ...
  for (i = 0; i < TIMES; i++) {
    ...
    x25519.randKey(alicesk);
    x25519.keygen(alicepk, alicesk);

    X25519_KEY bobsk, bobpk;
    x25519.randKey(bobsk);
    x25519.keygen(bobpk, bobsk);
