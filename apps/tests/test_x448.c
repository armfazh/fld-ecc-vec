...
static void test_ietf_cfrg0()
{
	...
	printf(LABEL,memcmp(output_coord0, shared,ECDH448_KEY_SIZE_BYTES)==0?OK:ERROR);
	...
	printf(LABEL,memcmp(output_coord1, shared,ECDH448_KEY_SIZE_BYTES)==0?OK:ERROR);
	...
	printf(LABEL,memcmp(alice_pk, alice_public,ECDH448_KEY_SIZE_BYTES)==0?OK:ERROR);
	...
	printf(LABEL,memcmp(bob_pk, bob_public,ECDH448_KEY_SIZE_BYTES)==0?OK:ERROR);
	...
	printf(LABEL,memcmp(alice_shared_secret, bob_shared_secret,ECDH448_KEY_SIZE_BYTES)==0?OK:ERROR);
	printf(LABEL,memcmp(alice_shared_secret, shared_secret,ECDH448_KEY_SIZE_BYTES)==0?OK:ERROR);
}
...
static void times(int n,uint8_t * k)
{
	...
	for(i=0;i<n;i++)
	{
		ECDHX.X448.shared(pR,pU,pK);
		uint8_t *tmp=pU;
		pU = pK;
		pK = pR;
		pR = tmp;
	}
	memcpy(k,pK,ECDH448_KEY_SIZE_BYTES);
}

static void test_ietf_cfrg1()
{
...
*/
	ECDH_X448_KEY k;
	times(1,k);
	printf(LABEL,memcmp(k, k_1_times,ECDH448_KEY_SIZE_BYTES)==0?OK:ERROR);
	times(1000,k);
	printf(LABEL,memcmp(k, k_1000_times,ECDH448_KEY_SIZE_BYTES)==0?OK:ERROR);
	/*times(1000000,k);
	printf(LABEL,memcmp(k, k_1000000_times,ECDH448_KEY_SIZE_BYTES)==0?OK:ERROR);*/
}


static void test_ietf_cfrg()
{
	test_ietf_cfrg0();
	test_ietf_cfrg1();
}

static void test_diffiehellman()
{
	...
	for (i = 0; i < TIMES; i++)
	{
		ECDH_X448_KEY alicesk, alicepk;
		random_bytes(alicesk,ECDH448_KEY_SIZE_BYTES);
		ECDHX.X448.keygen(alicepk, alicesk);

		ECDH_X448_KEY bobsk, bobpk;
		random_bytes(bobsk,ECDH448_KEY_SIZE_BYTES);
		ECDHX.X448.keygen(bobpk, bobsk);

		ECDH_X448_KEY alice_shared_secret;
		ECDH_X448_KEY bob_shared_secret;
		ECDHX.X448.shared(alice_shared_secret, bobpk, alicesk);
		ECDHX.X448.shared(bob_shared_secret, alicepk, bobsk);
		match = memcmp(alice_shared_secret, bob_shared_secret,ECDH448_KEY_SIZE_BYTES)==0;
		if(!match)
		{
			break;
		}
