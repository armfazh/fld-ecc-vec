...
{
	ECDH_X25519_KEY shared;

	ECDH_X25519_KEY key0 = {
			0xa5,0x46,0xe3,0x6b,0xf0,0x52,0x7c,0x9d,
			0x3b,0x16,0x15,0x4b,0x82,0x46,0x5e,0xdd,
			0x62,0x14,0x4c,0x0a,0xc1,0xfc,0x5a,0x18,
			0x50,0x6a,0x22,0x44,0xba,0x44,0x9a,0xc4};

	ECDH_X25519_KEY input_coord0 = {
			0xe6,0xdb,0x68,0x67,0x58,0x30,0x30,0xdb,
			0x35,0x94,0xc1,0xa4,0x24,0xb1,0x5f,0x7c,
			0x72,0x66,0x24,0xec,0x26,0xb3,0x35,0x3b,
			0x10,0xa9,0x03,0xa6,0xd0,0xab,0x1c,0x4c};

	ECDH_X25519_KEY output_coord0 = {
	...
	printf(LABEL,memcmp(output_coord0, shared,x25519->key_size)==0?OK:ERROR);

	ECDH_X25519_KEY key1 = {
			0x4b,0x66,0xe9,0xd4,0xd1,0xb4,0x67,0x3c,
			0x5a,0xd2,0x26,0x91,0x95,0x7d,0x6a,0xf5,
			0xc1,0x1b,0x64,0x21,0xe0,0xea,0x01,0xd4,
			0x2c,0xa4,0x16,0x9e,0x79,0x18,0xba,0x0d};

	ECDH_X25519_KEY input_coord1 = {
			0xe5,0x21,0x0f,0x12,0x78,0x68,0x11,0xd3,
			0xf4,0xb7,0x95,0x9d,0x05,0x38,0xae,0x2c,
			0x31,0xdb,0xe7,0x10,0x6f,0xc0,0x3c,0x3e,
			0xfc,0x4c,0xd5,0x49,0xc7,0x15,0xa4,0x93};

	ECDH_X25519_KEY output_coord1 = {
	...
	printf(LABEL,memcmp(output_coord1, shared,x25519->key_size)==0?OK:ERROR);

	ECDH_X25519_KEY alice_sk = {
			0x77,0x07,0x6d,0x0a,0x73,0x18,0xa5,0x7d,
			0x3c,0x16,0xc1,0x72,0x51,0xb2,0x66,0x45,
			0xdf,0x4c,0x2f,0x87,0xeb,0xc0,0x99,0x2a,
			0xb1,0x77,0xfb,0xa5,0x1d,0xb9,0x2c,0x2a};

	ECDH_X25519_KEY alice_pk = {
			0x85,0x20,0xf0,0x09,0x89,0x30,0xa7,0x54,
			0x74,0x8b,0x7d,0xdc,0xb4,0x3e,0xf7,0x5a,
			0x0d,0xbf,0x3a,0x0d,0x26,0x38,0x1a,0xf4,
			0xeb,0xa4,0xa9,0x8e,0xaa,0x9b,0x4e,0x6a};
	ECDH_X25519_KEY alice_public;

	x25519->keygen(alice_public,alice_sk);
	printf(LABEL,memcmp(alice_pk, alice_public,x25519->key_size)==0?OK:ERROR);

	ECDH_X25519_KEY bob_sk = {
			0x5d,0xab,0x08,0x7e,0x62,0x4a,0x8a,0x4b,
			0x79,0xe1,0x7f,0x8b,0x83,0x80,0x0e,0xe6,
			0x6f,0x3b,0xb1,0x29,0x26,0x18,0xb6,0xfd,
			0x1c,0x2f,0x8b,0x27,0xff,0x88,0xe0,0xeb};

	ECDH_X25519_KEY bob_pk = {
			0xde,0x9e,0xdb,0x7d,0x7b,0x7d,0xc1,0xb4,
			0xd3,0x5b,0x61,0xc2,0xec,0xe4,0x35,0x37,
			0x3f,0x83,0x43,0xc8,0x5b,0x78,0x67,0x4d,
			0xad,0xfc,0x7e,0x14,0x6f,0x88,0x2b,0x4f};

	ECDH_X25519_KEY bob_public;

	x25519->keygen(bob_public,bob_sk);
	printf(LABEL,memcmp(bob_pk, bob_public,x25519->key_size)==0?OK:ERROR);

	ECDH_X25519_KEY shared_secret = {
			0x4a,0x5d,0x9d,0x5b,0xa4,0xce,0x2d,0xe1,
			0x72,0x8e,0x3b,0xf4,0x80,0x35,0x0f,0x25,
			0xe0,0x7e,0x21,0xc9,0x47,0xd1,0x9e,0x33,
			0x76,0xf0,0x9b,0x3c,0x1e,0x16,0x17,0x42};

	ECDH_X25519_KEY alice_shared_secret;
	ECDH_X25519_KEY bob_shared_secret;

	x25519->shared(bob_shared_secret,alice_public,bob_sk);
	x25519->shared(alice_shared_secret,bob_public,alice_sk);
	printf(LABEL,memcmp(alice_shared_secret, bob_shared_secret,x25519->key_size)==0?OK:ERROR);
	printf(LABEL,memcmp(alice_shared_secret, shared_secret,x25519->key_size)==0?OK:ERROR);
}

static void test0_rfc7748_x448()
{
	ECDH_X448_KEY shared;
	ECDH_X448_KEY key0 = {
	...
	ECDHX.X448.shared(shared,input_coord0,key0);
	printf(LABEL,memcmp(output_coord0, shared,ECDHX.X448.key_size)==0?OK:ERROR);
	...
	ECDHX.X448.shared(shared,input_coord1,key1);
	printf(LABEL,memcmp(output_coord1, shared,ECDHX.X448.key_size)==0?OK:ERROR);

	ECDH_X448_KEY alice_sk = {
	...
	ECDH_X448_KEY alice_public;

	ECDHX.X448.keygen(alice_public,alice_sk);
	printf(LABEL,memcmp(alice_pk, alice_public,ECDHX.X448.key_size)==0?OK:ERROR);

	ECDH_X448_KEY bob_sk = {
	...
	ECDH_X448_KEY bob_public;

	ECDHX.X448.keygen(bob_public,bob_sk);
	printf(LABEL,memcmp(bob_pk, bob_public,ECDHX.X448.key_size)==0?OK:ERROR);
	...
	ECDH_X448_KEY alice_shared_secret;
	ECDH_X448_KEY bob_shared_secret;

	ECDHX.X448.shared(bob_shared_secret,alice_public,bob_sk);
	ECDHX.X448.shared(alice_shared_secret,bob_public,alice_sk);
	printf(LABEL,memcmp(alice_shared_secret, bob_shared_secret,ECDHX.X448.key_size)==0?OK:ERROR);
	printf(LABEL,memcmp(alice_shared_secret, shared_secret,ECDHX.X448.key_size)==0?OK:ERROR);
}
...
static void test1_rfc7748_x25519(const DiffieHellmanXFunction *x25519)
{
	...
	ECDH_X25519_KEY k;
	times(1,k,9,x25519);
	printf(LABEL,memcmp(k, k_1_times,x25519->key_size)==0?OK:ERROR);
	times(1000,k,9,x25519);
	printf(LABEL,memcmp(k, k_1000_times,x25519->key_size)==0?OK:ERROR);
/*	times(1000000,k,9,x25519);
	printf(LABEL,memcmp(k, k_1000000_times,x25519->key_size)==0?OK:ERROR);*/
}

static void test1_rfc7748_x448()
{
	ECDH_X448_KEY k_1_times = {\
	...
	ECDH_X448_KEY k;
	times(1,k,5,&ECDHX.X448);
	printf(LABEL,memcmp(k, k_1_times,ECDHX.X448.key_size)==0?OK:ERROR);
	times(1000,k,5,&ECDHX.X448);
	printf(LABEL,memcmp(k, k_1000_times,ECDHX.X448.key_size)==0?OK:ERROR);
	/*times(1000000,k,5,&ECDHX.X448);
	printf(LABEL,memcmp(k, k_1000000_times,ECDHX->X448->key_size)==0?OK:ERROR);*/
}

static void test_dh(const DiffieHellmanXFunction *ecdh)
{
